...
********************************************************************;
*        INPUT: Files generated from SAS program sasxpt_r.sas;
*               (run this program directly after sasxpt_r.sas);
********************************************************************;
...
********** Macro to read file in library tmpsas;
%macro Read(DBIN);
;/********** READ SAS FILE */ 
data &DBIN;
    set tmpsas.&DBIN;
...
********** Results from pipe were used to read all .sas7bdat files;
%macro ReadAll();
    %********** Invoke the %Read macro for each data set file;
    data _null_; 
        set db00_XPT;
        call execute('%Read('||trim(FILENAME)||');');
    run;
%mend ReadAll;

********** Execute final Macro for Reading the SAS files;
%ReadAll();
